# 你不知道的JS-摘录-this与对象原型-3

## 摘自第五章

:::details 写在前面
这个章节写的内容非常丰富和生动，能够得到很多启发，难度上并不会很大，但我还是对大部分内容都进行了完整的摘抄，如果想快速判断是否熟悉从而决定是否跳过，可以直接看 简单小结之类的部分。
:::
...
:::details 简单小结
对象天生带有原型链`Prototype`，可以通过`get`向上查找属性（ps:对象、数组用`.__proto__`，函数才有`prototype`，`.__proto__`会指向其构造函数的`prototype`）；尽头是 `Object.prototype`。
:::
...
:::details 总结与感想
这一段有很多需要注意的地方，关于原型链数据的读取与设置，隐式遮蔽问题。原型链上有属性，如果访问的对象有，那么就是最常见的访问和修改，如果是原型链上原型对象有，访问对象没有就会有遮蔽现象：

1. 如果一个普通的名为 `foo` 的数据访问属性在 `[[Prototype]]` 链的高层某处被找到，**而且没有被标记为只读（`writable:false`）**，那么一个名为 `foo` 的新属性就直接添加到 `myObject` 上，形成一个 **遮蔽属性**。
2. 如果一个 `foo` 在 `[[Prototype]]` 链的高层某处被找到，但是它被标记为 **只读（`writable:false`）** ，那么设置既存属性和在 `myObject` 上创建遮蔽属性都是 **不允许** 的。如果代码运行在 `strict mode` 下，一个错误会被抛出。否则，这个设置属性值的操作会被无声地忽略。不论怎样，**没有发生遮蔽**。
3. 如果一个 `foo` 在 `[[Prototype]]` 链的高层某处被找到，而且它是一个 setter（见第三章），那么这个 setter 总是被调用。没有 `foo` 会被添加到（也就是遮蔽在）`myObject` 上，这个 `foo` setter 也不会被重定义。

**遮蔽就是子“类”根据原型链上访问到的结果自己生成一个属性，以后读取和修改都在它身上进行；但是如果原型链上属性只读，那么不会产生遮蔽，不会被修改；如果原型链上属性是`setter`，那么修改时是调用原型链上的`set`方法，子“类”上不产生遮蔽**

我为这几个场景简单编写了代码，附带运行结果：
...
s.a // 1
s.hasOwnProperty("a") // false
s.a = 2
s.hasOwnProperty("a") // true
f.a // 1
```
可以看到子“类”产生遮蔽现象，不修改父“类”属性，属性在子类身上生成；
...
f.a=2
f.a // 1
let son = Object.create(f)
son.a // 1
son.hasOwnProperty("a") // false
son.a = 2
son.a // 1
son.hasOwnProperty("a") // false
```
可以看到属性不能够被修改，子“类”上也不产生遮蔽现象

场景三： 

重点在于`set a`，子“类”上最后也没有属性`"a"`，即不产生遮蔽；
```js
let father = {
    _a:1,
    set a(newV) {
...
son.hasOwnProperty("a") // false
```
但很容易意识到，`son`仍然拥有了`_a`产生了遮蔽，这看起来这个特性对日常开发没有什么影响，`son.hasOwnProperty("a")`是`true`或`false`并不值得多少关注；但这个例子也足够说明属性修改是调用父“类”的`set a`，父“类”定义了 get a / set a，a 就是 father 的自有属性；子“类”不产生遮蔽。
:::
...
:::details 省流
这部分就是讲，所有的函数默认都会得到一个公有的，不可枚举的属性，称为 `prototype`，它可以指向任意的对象。同时通过函数`new`创建的对象，链接到该函数的`prototype`（当然，正如我上面说过的，对象的`.__proto__`属性指向`Foo.prototype`）。神奇的是：`new Foo()` 函数调用实际上几乎和建立链接的处理没有任何 *直接* 关系。**它是某种偶然的副作用**。

字里行间依旧紧扣重要的核心思想：**“类”是拷贝，而`js`中，主要是*对象*与*链接*，这是最底层决定不同的关键**。

这里顺便总结一下下面的部分章节内容吧，感觉写的有些太冗余了：下面有章节不摘录，它的大致意思就是说 **“继承”是拷贝，数据流向画成图看起来应该是父类中的数据拷贝到子类一份** ；但是 **对于原型链** 来说，它其实 **是向上访问和`get`**，因此是一个查找的动作而不是数据流向， **查找的方向则是自下而上，从“子类”到“父类”**，两个情况方向是相反的。

构造器章节讲了`js`中一些让人受“构造器”困惑的设计（可以说是受“类”困惑，毕竟完全一样，`js`中没有类，当然不应该有类构造器）；其中提到：

1. 默认给了`.constructor`方法（通过对一个函数的`prototype`自动增加`.constructor`方法，并令其绑定自身，这样通过这个函数创建的所有对象都能访问到`.constructor`），但是 **“constructor”并不像它看起来的那样实际意味着“被XX创建”。**

2. 根据 JavaScript 世界中的惯例，“类”都以大写字母开头的单词命名。**注意：** 这个惯例是如此强大，以至于如果你在一个小写字母名称的方法上使用 `new` 调用，或并没有在一个大写字母开头的函数上使用 `new`，许多 JS 语法检查器将会报告错误。这是因为我们如此努力地想要在 JavaScript 中将（假的）“面向类” *搞对*，所以我们建立了这些语法规则来确保我们使用了大写字母，即便对 JS 引擎来讲，大写字母根本 **没有任何意义**。
:::
...
:::details 省流小结
这部分和我前面小结讲到的是一样的，当然摘抄的时候我省略了一些我认为比较简单的内容，那部分内容有点重复讲解了原型链的机制（对象上查不到，就沿着原型链查，自始至终都只是这样而已，很简单不是吗），然后引出了构造器这部分内容，正如上面小结所说：“（通过对一个函数的`prototype`自动增加`.constructor`方法，并令其绑定自身，这样通过这个函数创建的所有对象都能访问到`.constructor`）”；不过这一段我依然摘抄了下来，因为它还是提到了一个很重要的内容：**`.constructor` 不是一个魔法般不可变的属性。它是不可枚举的（见上面的代码段），但是它的值是可写的（可以改变）**，因此当我们真的是当做“类”在使用这些原型链内容，可能就会踩坑。
:::
...
:::details 小结
这部分内容相对来说看起来复杂一些，但是核心思想仍然不变，模仿类，**类是拷贝**，所以很简单，即使现在在讲原型链，也是在尝试原型链上“模仿拷贝”，怎么把父类的`Foo.prototype`为子“类”所用，并且像拷贝出来那样“分离”呢。所以引用赋值`Bar.prototype = Foo.prototype;`不行；`Bar.prototype = new Foo();`可以，但是同时因为执行了`new Foo()`，可能产生意料之外的副作用。因此，最后还是`Object.create(..)`；补充了`es6+`中引入的新方法：`Object.setPrototypeOf( Bar.prototype, Foo.prototype );`
:::
...
:::details 补充解释
感觉这部分写的不是很清晰，大概只是在问一个问题：“**在 `a` 的整个 `[[Prototype]]` 链中，有没有出现那个被 `Foo.prototype` 所随便指向的对象？**”。提供的两个写法其实都能解决这个问题，第一个方法只是内部会有一个看起来荒谬不合理的逻辑：`return o1 instanceof F;`,这里`F`跟`o1`从“类”理论上来说根本没有任何关系， `o1 instanceof F`返回结果却是`true`；第二种方法只是换了个`api`，直接接收对象而不需要函数，但本质和方法一是一样的，正如原文所说“上面的 `isRelatedTo(..)` 工具是内建在语言中的，它的名字叫 `isPrototypeOf(..)`”。

总的来说，**重点是怎么实现“类”关系的判断，然后顺便提到了`o1 instanceof F`返回结果是`true`的荒谬（而这就是`js`的特性导致没有类引发的）。**
:::
...
:::details 省流
这部分提到了我们早就知道的`__proto__`，它能从实例上访问到构造函数的`prototype`，即获取原型链。
:::
...
:::details 小结
这部分内容在讲解`Object.create()`的优秀之处，它为我们在对象间建立原型委托链接提供了完美的方式，并且没有其他乱七八糟的东西（就像前面讲过的`new`调用方法导致的副作用，搞乱 `.prototype` 和 `.constructor` 引用）。那么我想，可以得出一个`Object.create()`胜利的结论了，它能够基于原型链实现对象间的委托，完全不需要类的概念。（它基于原型链实现了对象间的委托链接，达到与其他语言中类继承相同的效果，但机制完全不同——`JS` 没有类，只有对象之间的链接和委托）

下面的内容就不是那么重要了，主要讲怎么在`es5`之前实现`Object.create()`，因为`Object.create()`是`es5+`引入的，不是特别重要，不过还是摘抄了下来，到`details`里折叠起来，可看可不看吧。这里再提一下下面折叠部分有意思的点：

1. `Object.create`的简单实现：
```js
if (!Object.create) {
...
```
2. 设计软件时可以处理一下原型链的情况，因为如果在 `myObject` 上没有 `cool()` 方法时调用 `myObject.cool()` 能工作，可能会使未来维护软件的开发者很吃惊，我们可以这样处理一下：
```js
var anotherObject = {
    cool: function() {
        console.log( "cool!" );
...
```
这样至少看的时候显然`myObject`是有`doCool`方法的。
:::

:::details 不太重要的——在`es5`之前实现`Object.create()`
#### 填补 `Object.create()`
...
:::details 核心
复习部分总结的很好很清晰，没有什么要补充的，最后强调一下核心观点吧：**类是拷贝，而`JS`中没有拷贝发生，对于`JS`而言，“委托”是一个更确切的术语，因为这些关系不是 *拷贝* 而是委托 **链接****
:::
...
覆盖操作不会引发新的依赖冲突：当依赖树中没有其他依赖会因为这次覆盖出现版本不兼容的情况时，npm会选择覆盖