...
<p>因为内容较多，在这里先讲解使用到的<code v-pre>export_json_to_excel</code>：</p>
<h3 id="export-json-to-excel" tabindex="-1"><a class="header-anchor" href="#export-json-to-excel"><span>export_json_to_excel</span></a></h3>
...
<p><code v-pre>write(wb, options)</code>：使用<code v-pre>xlsx</code>库将工作簿（<code v-pre>wb</code>）转为指定格式的二进制数据。
<code v-pre>s2ab(wbout)</code>：将二进制字符串转为<code v-pre>ArrayBuffer</code>，用于创建<code v-pre>Blob</code>对象。
<code v-pre>saveAs</code>：通过<code v-pre>file-saver</code>库触发浏览器下载。</p>
