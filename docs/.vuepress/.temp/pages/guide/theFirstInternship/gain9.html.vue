...
<p>项目五是一个<code v-pre>react</code>项目，功能可以说是一个外卖平台，我并没有系统学习过<code v-pre>react</code>，但恰巧开始学习<code v-pre>Taro+React</code>写小程序，因此也是有了更好的学习机会。</p>
...
<h2 id="外卖平台优惠券业务——需求与完成" tabindex="-1"><a class="header-anchor" href="#外卖平台优惠券业务——需求与完成"><span>外卖平台优惠券业务——需求与完成</span></a></h2>
<p>现在我需要写简历，因此要补充这部分业务的实现与问题了：</p>
<h3 id="优惠券页面构建" tabindex="-1"><a class="header-anchor" href="#优惠券页面构建"><span>优惠券页面构建</span></a></h3>
<p>需求需要在外卖系统构建一个优惠券路由页面，显示平台给用户发放的优惠券，并且可显示优惠券的状态：未使用、已使用、已过期；</p>
<p>页面可选择优惠券类型展示：全部、未使用、已使用、已过期；</p>
<p>可选择有效期展示：使用日期选择器选择时间范围，展示在对应时间范围内可用的优惠券；</p>
<p>对于SPA应用，多个页面样式总是有重复内容，因此参照着以前实现过的页面，很快可以构建出这个全新的页面</p>
<h3 id="优惠券对其他各个模块的影响处理" tabindex="-1"><a class="header-anchor" href="#优惠券对其他各个模块的影响处理"><span>优惠券对其他各个模块的影响处理</span></a></h3>
<p>另外，优惠券发放，就会出现用户购物时是否使用优惠券以及使用优惠券是什么类型、价格的实时计算与变化，订单金额是否达到使用优惠券的要求；</p>
<p>售后时，也要处理用户订单是否使用优惠券，原价和实际支付金额分别是多少，如果要撤销订单，申请退款又应该根据是否使用优惠券的不同情况进行不同操作；</p>
<p>因此全套业务需要考虑的内容并不少</p>
<p>对于这一系列问题，我和后端人员多次沟通交流共同确定方案，最终一步一步解决了这些问题：</p>
<h2 id="购物车付款页面" tabindex="-1"><a class="header-anchor" href="#购物车付款页面"><span>购物车付款页面</span></a></h2>
<p>在购物车付款页面，增加优惠券选项，增加实付金额和原金额的显示；</p>
<p>对于优惠券选项，下拉显示不使用优惠券、满10-5优惠券等优惠券类型，只有当金额达到对应价格时显示优惠券选项，比如满10元才会显示满10-5优惠券的选项；设置一个方法，传入优惠券信息，根据当前订单金额与优惠券使用金额匹配计算是否达到优惠券使用金额，返回布尔值决定在下拉选项中是否显示该优惠券。</p>
<p>对于实付金额的计算，由于跟优惠券优惠金额有关，需要根据原金额与优惠金额相减计算得到，通过前端实时匹配优惠金额实时计算实付金额，无需在用户选择、切换优惠券时发送请求获取金额，减小后端压力；</p>
<h2 id="订单页面" tabindex="-1"><a class="header-anchor" href="#订单页面"><span>订单页面</span></a></h2>
<p>订单页面包括未付款订单和已付款订单，对于未付款的订单点击处理，可以显示与购物车付款页面基本相同的内容，因此处理逻辑与上面基本相同。</p>
<p>对于已付款的订单主要是增加实付金额和原金额的显示，其他功能基本不变。</p>
</div></template>
