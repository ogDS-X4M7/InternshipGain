...
<p>简单来说，每个模块的功能都要尽可能做到可拆分，与无关的代码尽量减少联系，比如实现一个<code v-pre>a</code>功能，通常是编写一个组件，或者复用组件，为了实现功能可能会有很多的判断细节，可能判断的条件在在调用这个组件的父组件中，但是不应该把判断逻辑交给父组件，而是尽量都放到组件内实现，这就是解耦，其实也是为了尽可能地实现代码的可复用性，毕竟如果这个组件的<code v-pre>a</code>功能需要依托其父组件进行大量的判断，那么其他代码想要复用的话又得重复这部分判断，显然不合适。</p>
<h2 id="项目设计——方案评审与用户体验" tabindex="-1"><a class="header-anchor" href="#项目设计——方案评审与用户体验"><span>项目设计——方案评审与用户体验</span></a></h2>
<p>来到这边，自己也写过一些技术方案，参与了技术评审，其中遇到一个比较细节的点，大概就是<code v-pre>a</code>功能是判别用户点击或滑动来实现一个弹窗提醒，这是产品需求。</p>
...
<p>其实原理和逻辑我都懂，这也是我在遇到这个有些诡异的情况的时候最后能够想明白的原因。<code v-pre>transform</code>实现元素移动可以说是最高效的，因为<code v-pre>transform</code>既不会影响布局，也不会影响绘制指令。他是直接在画出画面的阶段完成。把内容渲染到页面上一般是要分成8个步骤，它包括<code v-pre>HTML</code>的解析。样式的计算、布局、分层、生成绘制指令、分块、光栅化和画。因此一些页面的元素进行移动旋转等操作的时候，<code v-pre>transform</code>是一种非常好的实现方式，因此也是非常常用的实现方式。但恰恰是它的这个特性导致了盒子的样式经历了<code v-pre>left:50%</code>之后就可以渲染，然后得到了一个最多只有屏幕宽度一半的盒子，并且通过<code v-pre>transform</code>移动居中之后盒子的宽度也是不变的，因为正如刚刚原理所说，它是指影响最后的一个步骤，也就是画的步骤，他并不干扰样式的计算，不干扰<code v-pre>dom</code>的布局。</p>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div></div></div><p>也是因为这个，我发现在<code v-pre>npm</code>包本地代码生成软链接时使用的<code v-pre>Node</code>版本和小程序的代码里使用的版本是不同的，这导致了引入时没有使用到正确的软链接，也就不是最新版本。所以及时统一到同样的<code v-pre>node</code>版本后项目就正常运行了。</p>
<h2 id="项目上线" tabindex="-1"><a class="header-anchor" href="#项目上线"><span>项目上线</span></a></h2>
<p>代码编写完成后将<code v-pre>npm</code>部分提交，经历<code v-pre>mr</code>合并并完成发布，就能够将更新到本地编写完成后的版本。</p>
<p>接着到小程序发布的步骤，因为小程序引用依赖了<code v-pre>npm</code>包，所以需要先发布<code v-pre>npm</code>包。然后将依赖即<code v-pre>package.json</code>中的<code v-pre>npm</code>依赖版本更新到发布后的最新版本。<strong>npm的最新版本查看的命令是：</strong><code v-pre>npm view &lt;package_name&gt; versions</code>，可以通过这条命令查看自己的npm包是否确实发布成功。</p>
<p>然后重新安装依赖，可以在本地测试一下安装最新版<code v-pre>npm</code>包后的小程序运行起来是否正常，检查完成后就可以把小程序也提交，同样经历<code v-pre>mr</code>合并，到这里我们的任务就完成了，最后发布线上、运行就不需要我们再管了。</p>
<h3 id="冲突与合并的处理" tabindex="-1"><a class="header-anchor" href="#冲突与合并的处理"><span>冲突与合并的处理</span></a></h3>
<p>由于同一个项目会有大量的开发人员共同开发，各自的代码合并进入主分支的时间并不相同，所以较晚合并代码的开发人员需要从主分支更新其他开发人员更新的代码，并且处理相应的冲突（如果有的话）。一般来说，如果需要共同处理同一部分的代码，那么会将自己这边提交的更改暂存起来，然后更新同步主分支代码。之后再将自己的更改重新添加到本地代码中。而如果没有需要共同处理的代码，那么一般会发生冲突的只有<code v-pre>package.json</code>和<code v-pre>package-lock.json</code>。这两个文件冲突，那么当然就是以主分支的为准，解决冲突后，合并主分支完成，需要重新根据主分支的<code v-pre>package.json</code>安装依赖，因为重新安装依赖会清空之前的<code v-pre>npm link</code>，所以重新安装依赖之后还需要重新<code v-pre>npm link</code>。</p>
<h3 id="插曲——ts类型修复" tabindex="-1"><a class="header-anchor" href="#插曲——ts类型修复"><span>插曲——ts类型修复</span></a></h3>
<p>当我合并主分支并尝试提交自己完成的代码时，发生过一个小插曲：个人的代码提交时一切顺利，只有报告与主分支代码冲突，导致流水线暂停。于是我关闭流水线重新合并主分支代码并提交，结果发生了<code v-pre>eslint</code>检测不通过的问题；这很奇怪，这意味着目前的主分支代码存在不能通过<code v-pre>eslint</code>校验的内容。但是为什么别人没有通过校验的代码能够提交到主分支上并不是我需要关心的最主要的问题。我需要找出不符合规范的地方，并且进行处理。</p>
<p>这里讲一下我当时使用的<strong>TypeScript 类型检查命令：</strong><code v-pre>npx tsc -p tsconfig.json --noEmit</code>，这条指令能够基于指定的<code v-pre>tsconfig.json</code>配置文件，对项目进行类型检查，但不生成编译后的<code v-pre>JavaScript</code>文件；它的输出结果能够直接定位不符合规范的地方。</p>
<p>最后定位到了一个很简单的问题。跟<code v-pre>ts</code>自动推断有关，一个被赋值了的字符串变量后有开发人员给他类型注解了变量类型为<code v-pre>string</code>，由于现在<code v-pre>ts</code>对于简单变量类型能够自动推断，因此这个注解是多余的，而<code v-pre>eslint</code>的检测并不允许这种多余的注解的存在，因此出现了这个问题。那么解决方法很简单，把这个多余的注解删掉就行了。当然这也提醒我，基础的知识还是要记牢，<code v-pre>eslint</code>的检测也不可少。</p>
<h2 id="对-壳-的认识" tabindex="-1"><a class="header-anchor" href="#对-壳-的认识"><span>对“壳”的认识</span></a></h2>
...
<p>最后讲一下<code v-pre>npm</code>依赖的问题。正如前面介绍过的，会通过包装成<code v-pre>npm</code>包的形式，让小程序按需引入，那么就会有很多的功能被拆分开来，拆分成非常多的<code v-pre>npm</code>包，然后被放到一个统一的大型项目里。</p>
<p>那么每个<code v-pre>npm</code>包就是一个小型项目，它们也有自己各自的依赖。但是由于他们并不是一同开发，是各自作为一个小项目，因此他们的依赖都是不受限的，那么就很容易出现同样需要某个依赖，但是他们使用了同个依赖的不同版本。那么对于引入了他们的项目，比如小程序，那么小程序就需要去下载同个依赖的不同版本，显然造成了不必要的浪费，导致项目的体积膨胀。这个问题我们管它叫做<strong>npm版本不统一导致安装多版本依赖而产生的体积膨胀问题</strong>。</p>
<h3 id="npm版本统一的实现与风险" tabindex="-1"><a class="header-anchor" href="#npm版本统一的实现与风险"><span>npm版本统一的实现与风险</span></a></h3>
<p>那么很明显为了解决上述的问题，我们需要统一<code v-pre>npm</code>的版本。前面所讲到的“壳”其实就对这方面做了一定的限制。正如前面所介绍壳是各个包外面的一个容器。因此它可以实现对各个包的依赖，进行一个统一的管理。壳也有自己的依赖，可以说是有<code v-pre>package.json</code>和<code v-pre>package-lock.json</code>，他们会统一项目内各个包的依赖，其他项目在引用这些包时，首先从壳内对所需的版本进行获取、安装，但是可也并没有对所有的依赖都进行了约束，而且对于有些依赖也是采取了<code v-pre>“1.0.2</code>的写法，这种写法意味着依赖的安装版本仍然是灵活的，并没有受到完全严格的限制，这都是有原因的。</p>
<p>对于<code v-pre>npm</code>的版本统一，理想的情况下，我们可以对于各个包的依赖都进行检查，并且全部统一到壳的依赖中，统一采取这些依赖的最新版本，并且采用严格精确的版本进行限制。这样大刀阔斧的改革，其实能够从根源上解决问题，但是对于一个运作多年的项目来说，是具备风险的。</p>
<p>因为依赖的不同版本是可能存在冲突的，一个旧版本的特性在新的版本中不一定能够得到保留，如果说我们的某个组件需要使用一个较旧版本的特性，当我们把它更新到一个较新的版本，就可能导致这个组件不能够正常地工作。其实我们做严格精确的版本限制就是对依赖版本更新有可能存在这种风险所进行的预防，所以像刚刚讲到的统一所有包的依赖到最新的版本，具体说是到我们使用的所有版本中最新的一个，它也是有像上面讲到的这种导致使用旧版本依赖的组件功能不可用的风险的。并且对于一个内容庞杂，大型的项目来说，是很有可能出现这种问题的。所以说如果要进行这样的更新改革那么不管是测试也好，开发调整也好，都可能会有非常巨大的工作量。并且最终的结果可能仍然需要采用多版本的依赖。所以说<code v-pre>npm</code>版本的统一，是一个困难的、理想化的要求，并不是说他不可能实现，但是实现的过程一定是耗费人力物力，并且可能达不到理想结果的。</p>
<p>所以说目前壳的管理对实现<code v-pre>npm</code>版本统一的角度来说，应该是达到了最理想的状况了。所以关于这部分的内容，我们<strong>最关心的还是npm版本自动更新的风险</strong></p>
<h3 id="npm版本自动更新的风险与预防" tabindex="-1"><a class="header-anchor" href="#npm版本自动更新的风险与预防"><span>npm版本自动更新的风险与预防</span></a></h3>
<p>正如前面所说，有些依赖对安装版本采取了灵活的写法（<code v-pre>^1.0.2</code>），这种写法意味着如果存在比<code v-pre>1.0.2</code>更高的版本，那么项目在依赖安装时可能会安装更新的版本，并且它的更新是自动的、不易察觉的。而正如我们刚刚所讲一个依赖的新的版本，与旧的版本可能是存在冲突的。如果说依赖在不被察觉的情况下自动更新，那么有可能以前的项目功能代码会失效。因此如果出现依赖版本更新，开发必须要察觉了解并且进行风险评估，确保版本更新不影响项目的运行。</p>
<p>所以我们需要建设<code v-pre>npm</code>依赖变动识别的能力。</p>
</div></template>
