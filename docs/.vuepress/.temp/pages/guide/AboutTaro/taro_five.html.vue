<template><div><h1 id="个人页——支持登录-更新头像昵称-查看历史、点赞、收藏-支持退出登录" tabindex="-1"><a class="header-anchor" href="#个人页——支持登录-更新头像昵称-查看历史、点赞、收藏-支持退出登录"><span>个人页——支持登录，更新头像昵称，查看历史、点赞、收藏，支持退出登录</span></a></h1>
<h2 id="登录与微信绑定" tabindex="-1"><a class="header-anchor" href="#登录与微信绑定"><span>登录与微信绑定</span></a></h2>
<p>个人页的功能可以说是这个项目里最复杂的一部分，用户的登录是直接和微信绑定的，由于微信登录的各种限制和当前微信头像，昵称获取方法的不断调整，在这个过程中有很多的问题需要小心和避免。</p>
<p>首先要使用微信登录需要用户的授权，而且这个授权必须要用户确认。之前使用的API已经弃用，<strong>具体的内容可以去看源代码里面me.jsx的useEffect中注释掉的部分，有对应代码和讲解</strong>。</p>
<p>微信用户的唯一id是他内部的一个openid，那我们获取这个open ID需要向微信官方提供的一个对应链接发送请求并且在链接中携带对应的参数。包括APP ID、APP secret和临时code，其中APP ID和secret也就是小程序的ID和小程序密钥，在微信平台登陆后可以查看获取，后端配置在文件里就可以了。但是临时code就需要用户确认获取。因此需要做一个登录的弹窗，让用户确认授权。来获取这个临时code。然后将这个临时code传给后端，后端根据APP ID，APP secret和临时code向微信官方链接发送请求来获取微信账号对应的唯一的openID。通过这个open ID后端能够判断用户是否已有账号，并且返回这个账号对应的信息，如果用户没有注册过账号，则根据他的open ID生成注册一个新的账号并返还给用户，同时还会生成token返回。</p>
<p>前端需要根据后端返回来的账号信息更新页面的内容，用户未登陆时页面只有一个提示登录的卡片，登陆后则有用户的头像，昵称卡片，以及个人页所有的功能界面按钮。因此需要更新界面原本有的功能，并且根据信息显示用户头像，昵称，用户登录这里还包括一个token的存储，并且根据token可以实现自动登录，当然如果token失效则需要重新登陆。我们之前提到的用户收藏需要准确校验。在用户登陆后也需要根据token及时向后端发送请求来获取更新用户的收藏信息。</p>
<p>个人页面的设计上遇到过很多问题，先来看一下整个页面的代码：</p>
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code><span class="line">&lt;View className='me' ref={ref}></span>
...
<p>在OpenModal函数中添加了e.stopPropagation()；将onClick改为onClickCapture；</p>
<p>这两个修改都与 React/Taro 的事件处理机制密切相关。冒泡和捕获是基础的概念，不赘述：</p>
<p>由于内外层的View都有点击事件，这会导致两个问题：</p>
<ol>
<li>
<p>事件冲突：内外层的点击事件相互干扰；</p>
</li>
<li>
<p>事件阻塞：外层事件可能阻止内层事件的正常处理</p>
</li>
</ol>
<p>显然这跟之前第一个问题是有关系，尽管事件监听的确被移除，但是也确实有相应的警告，也许因为监听名义上真的还存在，导致出现了一些阻塞的干扰，因此，当我在内部使用onclick，冒泡，随后被
外层的onclick阻塞住。</p>
<p>因为onclick默认是冒泡阶段处理的，因此内部改为onClickCapture，并不难理解，顾名思义，把它改为捕获阶段执行，那么，就不会等冒泡的时候阻塞了；</p>
<p>当然，因为外部移除本身存在问题，所以它不应该被激活，那么就应该给外部设置阻止内部冒泡：</p>
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code><span class="line">const OpenModal = useCallback((e) => {</span>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>e是内部的事件，也就是内部onclick被点击，会因为e.stopPropagation();而不冒泡，从而不激活外部的onclick，这样也避免出现阻塞；</p>
<p>一个阻止冒泡激活外部有故障的onclick；另一个把执行阶段调整到捕获阶段，这样即使冒泡被阻止也能正常激活内部的onclick。这个问题就解决了</p>
<p>至于阻塞后，点击外部配置的按钮为什么能释放，原因可能比较复杂：</p>
<p>这涉及<strong>事件队列清空与批量处理机制</strong>：</p>
<p>当点击另一个元素时，会触发新的事件流，此时可能触发队列清空机制；其中最有可能的就是React 的合成事件系统，它会在更新阶段统一处理事件队列</p>
<p><strong>合成事件的底层实现</strong>：</p>
<p>React 为了性能优化，将原生事件（如 click）包装成合成事件（SyntheticEvent），并统一绑定到根节点（document 或 container）。所有事件触发后不会立即执行，而是先存入 React 维护的事件队列，等待 React 的更新周期统一处理。</p>
<p><strong>事件队列的清空与批量执行逻辑</strong>：</p>
<p>当点击新元素时，会触发以下流程：</p>
<p>新事件入队：React 捕获到新点击事件，生成合成事件并加入队列。</p>
<p>批量更新机制：React 在事件处理阶段（如 componentDidUpdate 前）会开启一个 事务（Transaction），将队列中的所有事件批量执行。</p>
<p>队列清空：事件处理完毕后，队列会被清空，避免重复执行。</p>
<p><strong>案例说明</strong>：</p>
<p>假设快速点击元素 A 三次，再点击元素 B：点击 A 时，三次点击事件被存入 React 的事件队列，但未立即执行（因 React 异步更新特性）。点击 B 时，React 触发新的事件处理流程，检测到队列中有未处理的事件（三次 A 的点击），会将 A 的三次事件和 B 的事件一起批量执行，表现为 “积攒事件一次性触发”。</p>
<p><strong>总结分析：</strong></p>
<p>因此，之前冒泡而导致阻塞，外部onclick事件虽然造成阻塞，但阻塞到后面，点击其他事件，就进入批量执行了；批量执行显然是忽视了阻塞的外部onclick，由于前面特殊的操作，显然这个名存实亡的奇特事件的处理流程也变得不同寻常</p>
<h2 id="支持更新用户头像昵称-支持获取微信头像昵称" tabindex="-1"><a class="header-anchor" href="#支持更新用户头像昵称-支持获取微信头像昵称"><span>支持更新用户头像昵称，支持获取微信头像昵称</span></a></h2>
<p>前面提过，微信之前的获取用户信息的API废弃，现在是把获取用户信息的功能也废弃了，转而改为了这种获取用户头像昵称的特定写法比如这里的open-type和input里面的type、Class，这些新的使用方法由微信官方提供，在官方文档可以查看。需要注意的是微信提供的特定解法是针对原生组件的，像这里就曾经尝试过使用Taro的按钮组件，最后并没有成功获取用户微信的信息。</p>
<p>这里使用AtDrawer抽屉承载这部分功能：</p>
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code><span class="line">&lt;AtDrawer</span>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="支持查看历史浏览、点赞、收藏——三功能代码复用的区分" tabindex="-1"><a class="header-anchor" href="#支持查看历史浏览、点赞、收藏——三功能代码复用的区分"><span>支持查看历史浏览、点赞、收藏——三功能代码复用的区分</span></a></h2>
<p>查看历史浏览，点赞，收藏的功能都是高度近似的。因此新建一个视频列表页可以供这三个功能复用。区分这三个页面的内容除了调用对应的三个函数，还在页面跳转的同时在链接中传递了标志参数，用于调整不同类型视频列表页的样式，如页面标题。</p>
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code><span class="line">// grid设置宫格对应方法</span>
...
<span class="line"></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="支持查看历史浏览、点赞、收藏——三功能代码复用的实现" tabindex="-1"><a class="header-anchor" href="#支持查看历史浏览、点赞、收藏——三功能代码复用的实现"><span>支持查看历史浏览、点赞、收藏——三功能代码复用的实现</span></a></h2>
<p>上面提到的参数，在视频列表页的相关实现是：</p>
<p>在useEffect中获取链接的params参数，判别是哪个视频列表页要展示，从仓库中获取该页面要展示的视频列表，底部样式根据参数展示不同内容</p>
<h3 id="视频列表页——useeffect的回调函数是async产生报错" tabindex="-1"><a class="header-anchor" href="#视频列表页——useeffect的回调函数是async产生报错"><span>视频列表页——useEffect的回调函数是async产生报错</span></a></h3>
<p>（视频列表页是额外设计的一个页面，但是它不是像首页、短视频播放页、个人页这些tab页。因此虽然它是一个独立的页面，但是由于与个人页的功能紧密相关，放在个人页的介绍内一并讲解）</p>
<p>在这里useEffect在实现的时候尝试过使用async回调函数，这了一些意料之外的bug：</p>
<p>如果useEffect的回调函数如果直接使用async会导致，页面能进入，但是退出时会报错c is not a function，程序崩溃：</p>
<p>原因如下：async 函数会隐式返回一个 Promise，而 useEffect 期望的返回值是一个清理函数（或不返回任何值）；因此，当组件卸载时，React 会发现拿到一个promise返回，导致程序崩溃；</p>
<p>当然也可能是 组件卸载时异步操作尚未完成，导致资源释放异常；</p>
<p>总而言之,useEffect 的回调函数不能是 async；如果实在需要异步,那也应该在回调内部写async,而不能让回调成为async</p>
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code><span class="line">const HistoryVideo = forwardRef(({ videoStore }, ref) => {</span>
<span class="line">    ......</span>
...
<span class="line">})</span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="视频列表页——点击列表中项目跳转播放页——播放页的复用" tabindex="-1"><a class="header-anchor" href="#视频列表页——点击列表中项目跳转播放页——播放页的复用"><span>视频列表页——点击列表中项目跳转播放页——播放页的复用</span></a></h3>
<p>用户点击列表中的视频查看时，跳转对应的视频播放页同样可以复用之前的短视频播放页。注意复用的短视频播放页是tab页，所以需要使用switchTab。</p>
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code><span class="line">async function handleClickVideo(clickUrl) {</span>
...
<p>其实在之前的代码已经展示过，这里的useEffect是根据refresh刷新信号来判断页面的重载，是因为除了页面的重新挂载，还有一个清空历史记录的功能需要刷新页面的内容。</p>
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code><span class="line">const HistoryVideo = forwardRef(({ videoStore }, ref) => {</span>
...
<span class="line">})</span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="支持退出登录" tabindex="-1"><a class="header-anchor" href="#支持退出登录"><span>支持退出登录</span></a></h2>
<p>点击退出登录时显示弹窗提示，用户确认退出，则移除存储的token，并且重新设置权限信号为true，展示无权限时的页面内容，当然最后关闭弹窗（设置弹窗显示信号为false）</p>
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code><span class="line">// 退出登录</span>
...
<span class="line">/></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="支持自动登录——个人页展示登录态内容-自动使用登录后功能" tabindex="-1"><a class="header-anchor" href="#支持自动登录——个人页展示登录态内容-自动使用登录后功能"><span>支持自动登录——个人页展示登录态内容，自动使用登录后功能</span></a></h2>
<p>项目实现了自动登录功能，登陆态和未登陆态的区别最主要是用户账号本地存储是否有token：一些需要登陆后使用的功能是通过发送请求时携带本地存储的token，由服务器校验后返回对应的请求结果实现。对于像个人页这种登录和未登录态存在显示内容上的区别的页面。则通过设置权限信号来区分两种页面的内容。自动登录则是在进入页面时检测本地存储的token。如果本地存储有token，则携带该token发送登录请求，自动登录请求成功则修改权限信号展示登录页面。自动登录请求失败，一般是因为用户本地存储的token过期，则提示登录状态过期，提示用户自行操作重新登陆。</p>
<p>下面是个人页自动登录并展示登录后页面的实现：</p>
<p>在页面挂载时尝试获取用户本地存储的token，如果能够获取则尝试自动登录。登陆成功则更新显示用户信息。</p>
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre v-pre><code><span class="line">const Me = forwardRef(({ userStore, videoStore }, ref) => {</span>
...
<span class="line">......</span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="注意setstate的异步更新" tabindex="-1"><a class="header-anchor" href="#注意setstate的异步更新"><span>注意setState的异步更新</span></a></h3>
<p>注意到这里抽屉和个人页都存有一份用户头像和昵称的信息。当自动登录成功获取用户信息后，需要将得到的信息分别交给抽屉和个人页，不能先设置个人页，然后将个人页的信息交给抽屉。也不能先设置抽屉，然后将抽屉的信息交给个人页。这是因为<strong>react的setstate的更新是异步执行的</strong>，如果像刚刚说的那样操作那么设置完之后的个人页或者抽屉信息是并没有实时更新、同步更新的，这个时候再将它们交给对方就会出现对方只能拿到还未完成设置的空白信息，所以说必须将自动登录请求成功后得到的信息分别交给抽屉和个人页。</p>
<p><strong>setState 的异步更新会在当前代码块执行完毕后、下一次 DOM 更新周期前触发</strong></p>
<p>上面的表述可能还不够严谨细致专业化，<strong>准确来说</strong>：微信小程序<strong>setState的渲染</strong>，并且<strong>setState的同个词法作用域闭包</strong>下<strong>捕获的是其渲染前的值</strong>，<strong>异步渲染后不更新</strong>，因此<strong>永远不要立刻使用刚setState设置完的值</strong>，要使用的话使用其数据源，而不是state：
和上面也可以说是一个意思，就比如先set个人页，然后set抽屉，个人页捕获请求值，但抽屉捕获了空值，最后异步渲染的时候，个人页当然拿到正常的请求值正常更新，但它自己更新不会影响当时抽屉已经捕获的空白的值，因此抽屉仍然拿着当初捕获的空值去更新自己，那最后更新完就不可能是想要的请求值了。</p>
<h3 id="自动登录的后台实现" tabindex="-1"><a class="header-anchor" href="#自动登录的后台实现"><span>自动登录的后台实现</span></a></h3>
<p>当然，自动登录很重要的部分还跟后端有关，因为需要提供根据token查询唯一用户的功能</p>
<p>我主要是通过使用cache，也就是redis缓存用户信息，并设置为可以通过token获取用户id、头像、昵称，因此可以使用token发送请求获取用户信息实现自动登录。</p>
