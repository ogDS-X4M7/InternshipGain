<template><div><h1 id="代码的简单整体解读" tabindex="-1"><a class="header-anchor" href="#代码的简单整体解读"><span>代码的简单整体解读</span></a></h1>
<p>首先得熟悉一下项目的代码都是怎么写的，才能把项目改成自己想要的样子，从docs项目文件夹来看</p>
<h2 id="docs文件夹的内容" tabindex="-1"><a class="header-anchor" href="#docs文件夹的内容"><span>docs文件夹的内容</span></a></h2>
<p>docs就是文档内容；docs目录下的README.md就是首页</p>
<h3 id="vuepress-config-js" tabindex="-1"><a class="header-anchor" href="#vuepress-config-js"><span>.vuepress/config.js</span></a></h3>
<p>.vuepress/config.js是进行总体设计，布局框架，可以理解为SPA(单页应用)的总体布局框架，也就是切换路由的时候内部内容会修改，但是外面的(比如顶部的边框一直都是一样的)这部分内容就是不变的框架，后面会提到的我当时对侧边栏的误解，就是因为config是对内部各个页面的侧边栏做的总体设计，当时误以为是对首页的设计了</p>
<p>根据官网给出的新项目，以及刚刚讲的config.js的功能，可以看到总体框架的设计是怎么样的：</p>
...
<li>
<p>title是整个项目的标题(显示在首页、顶部栏、标签页名称)</p>
</li>
<li>
<p>description是首页副标题，或者补充介绍</p>
</li>
<li>
<p>theme是主题，有默认主题、hope等，但我只是简单地实现文档网页，就直接用默认主题，</p>
</li>
<li>
<p>bundler:viteBundler():用于配置 Vite 作为打包工具，这个也是生成的初始项目就已经写好的内容</p>
</li>
</ol>
<h4 id="defaulttheme" tabindex="-1"><a class="header-anchor" href="#defaulttheme"><span>defaultTheme</span></a></h4>
<p>defaultTheme里面设置了总体、框架的内容，这部分比较重要，详细写一下：</p>
<p>4.1 navbar：顶部栏右上方的各个选项，数组存放对象的形式，一般就简单链接点击就跳转，和下拉栏两种类型
简单链接填写好显示的名字和跳转链接即可：{ text: '首页', link: '/' },
下拉菜单则是填好显示名称，将下拉要显示的内容放入children里，children内部的内容则与简单链接写法一致：</p>
...
<p>内部属性：</p>
<p>text设置目录名称，</p>
<p>collapsible设置是否可折叠，不可折叠的话就会全部列出来而不能收缩起来，因此一般设置true，</p>
<p>prefix设置前缀路径，</p>
<p>children则是这个分类所拥有的目录项——md文件，并且后缀(.md、.html)可省略</p>
<h4 id="不太重要的小内容" tabindex="-1"><a class="header-anchor" href="#不太重要的小内容"><span>不太重要的小内容</span></a></h4>
<p>毕竟聊到.vuepress/config.js了，就顺便写一下关于favicon图标修改的实现，是在.vuepress下创建public文件夹，里面存放好准备好的favicon图标，
随后在.vuepress/config.js里的export default defineUserConfig中添加设置即可</p>
