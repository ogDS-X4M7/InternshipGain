...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这个方案的核心是禁用 Webpack5 的预构建功能，其解决问题的逻辑如下：</p><p>关闭预构建的直接效果：</p><p>模块加载顺序恢复正常：所有模块按运行时需求动态加载，避免时序错误</p><p>模板解析回归动态模式：Taro 在运行时按需解析模板引用，确保tmpl_0_13被正确查找</p><p>组件样式作用域修复：关闭预构建后，组件样式能正确注入到页面作用域</p><p>配置参数的具体作用：</p><p>type: &#39;webpack5&#39;：保持使用 Webpack5 编译器，确保新特性支持</p><p>prebundle.enable: false：禁用预构建功能，这是解决问题的关键</p><p>prebundle.include：虽然启用了 include，但由于 enable 设为 false，该配置实际不生效</p><p>更深层的技术原理：</p><p>预构建本质是为了优化性能，但在复杂项目中可能破坏动态依赖关系</p><p>Taro 的模板系统依赖运行时动态解析，预构建的静态分析无法处理所有动态引用场景</p><p>关闭预构建后，Webpack 回归到传统的按需打包模式，确保模块和模板的动态加载能力</p><p><strong>没来得及记录的问题和内容</strong> 因为太过忙碌,总是来不及记录学习内容,在这下面先列举吧:</p><p>react中useEffect回调函数使用async导致的崩溃问题、以及排查的方法与过程</p><p>数据库实现浏览历史的数组操作问题、去重、</p><p>利用token在redis里拿到userid、</p><p>查询用户使用的userid是ObjectId类型，先转换才能查，转换方式：const userid = mongoose.Types.ObjectId(data.userid);</p><p>微信小程序使用用户微信头像、昵称的事件需绑定在原生组件上（button、input）</p><p>微信小程序setState的渲染，并且setState的同个词法作用域闭包下捕获的是其渲染前的值，异步渲染后不更新，因此永远不要立刻使用刚setState设置完的值，要使用的话使用其数据源，而不是state</p><p>报错</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre><code><span class="line">[MobX] Since strict-mode is enabled, changing (observed) observable values without using an action is not allowed. Tried to modify: ObservableObject@4.likeSignal</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div></div></div><p>原因是仓库中修改this.likeSignal没有在action中修改，解决方法： 将所有修改状态的语句都用runInAction(()=&gt;{xxx})包起来即可</p>`,78)]))}const d=n(l,[["render",p]]),r=JSON.parse('{"path":"/guide/AboutTaro/","title":"Taro 小程序+React学习","lang":"zh-CN","frontmatter":{},"headers":[],"git":{"updatedTime":1750524888000,"contributors":[{"name":"李锐标","username":"","email":"1415808154@qq.com","commits":2},{"name":"X4M7","username":"X4M7","email":"1415808154@qq.com","commits":2,"url":"https://github.com/X4M7"}],"changelog":[{"hash":"e43bab961abd39d293883f88cdbb1b492eb3e307","time":1750524888000,"email":"1415808154@qq.com","author":"X4M7","message":"略记无时间详解收获"},{"hash":"42c34a34f2b7a907e8a01bdd5e0e6eef4ef98611","time":1750468946000,"email":"1415808154@qq.com","author":"李锐标","message":"记录没时间详解的收获"},{"hash":"6f04d3ffa0f5ae82e07d6618a3dd6590808ce70f","time":1750086280000,"email":"1415808154@qq.com","author":"X4M7","message":"补充taro预构建导致问题解决方案"},{"hash":"602e0b1be38e4c5febc7f723fe34af31371125d9","time":1749549540000,"email":"1415808154@qq.com","author":"李锐标","message":"记录taro的学习"}]},"filePathRelative":"guide/AboutTaro/README.md"}');export{d as comp,r as data};
