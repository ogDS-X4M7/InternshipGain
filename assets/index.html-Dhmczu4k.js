import{_ as n,c as e,d as a,o as i}from"./app-CyvKfxuo.js";const l={};function c(p,s){return i(),e("div",null,s[0]||(s[0]=[a(`<h1 id="taro小程序-react——基础知识与正常运行须知" tabindex="-1"><a class="header-anchor" href="#taro小程序-react——基础知识与正常运行须知"><span>Taro小程序+React——基础知识与正常运行须知</span></a></h1><p>这部分学习内容结合相应代码查看，目的是复习和记下使用方法，只供个人查看</p><p>大部分写法、使用方法都直接看代码就能了解，只做稍微的记录：</p><p>有类组件和函数组件两种写法，常用函数组件，两个写法都很简单，直接去看代码就知道，这里不多说。组件的调用很简单，到对应页面直接<code>import</code>，在<code>return</code>里直接使用即可</p><p>样式也很简单，基本没变，在组件、页面同目录下创建文件，然后<code>import &#39;./xxx.scss&#39;</code>引入即可</p><h2 id="状态管理-重点" tabindex="-1"><a class="header-anchor" href="#状态管理-重点"><span>状态管理（重点）</span></a></h2><p>状态管理仓库使用<code>mobx</code>，<code>store</code>文件夹下创建想要的仓库<code>js</code>文件，引入<code>mobx</code>：</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre><code><span class="line">import { observable } from &#39;mobx&#39;</span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="发送请求-重点" tabindex="-1"><a class="header-anchor" href="#发送请求-重点"><span>发送请求（重点）</span></a></h2><p>发送请求<code>Taro</code>有自己的<code>Taro.request</code>，引入一下就能用：</p><p>这里是二次封装，设置网络请求拦截器，添加了请求时的顶部栏加载动画与请求后的关闭，添加了请求附带token的设置；</p><p>创建<code>service</code>文件夹，下创<code>httpService.js</code>:注意<code>return</code></p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre><code><span class="line">import Taro from &quot;@tarojs/taro&quot;</span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="webpack5-预构建机制导致的问题" tabindex="-1"><a class="header-anchor" href="#webpack5-预构建机制导致的问题"><span>Webpack5 预构建机制导致的问题</span></a></h2><p>遇到一个之前让我放弃的问题，没想到开发了这么久又突然冒了出来，这次我找到了解决方案：</p><p>问题： 报警告：</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre><code><span class="line">WXMLRT_$gwx:./base.wxml:template:213:16: Template \`tmpl_0_13\` not found.</span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>并且出现页面<code>AtModal</code>打开确认、取消按钮不显示的问题；</p><p>解决方案与流程： 找到<code>github</code>上<code>taro</code>的<code>issues</code>中有人提到<code>Taro3.6.25 开启 Prebundle 微信小程序会报 Template tmpl_0_13 not found 警告 #15493</code></p><p>可以通过搜索引擎搜索<code>Template &#39;tmpl_0_13&#39; not found</code>找到；</p><p>解决方案： 打开<code>config/index.js</code>,将</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre><code><span class="line">compiler: &#39;webpack5&#39;,</span>
...
<span class="line">    },</span>
<span class="line">  },</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>重新编译运行代码，重新打开微信开发者工具，代码恢复正常</p><p>问题的核心原因与<code>Webpack5</code>预构建机制有关：</p><p><code>Webpack5</code>预构建的影响：</p><p>当配置<code>compiler: &#39;webpack5&#39;</code>时，<code>Taro</code>默认启用了<code>prebundle（预构建）</code>功能</p><p>预构建会提前打包某些模块（如<code>@taro/components</code>），但可能导致以下问题：</p><p>模块加载顺序混乱：预构建模块与业务模块的加载时序不匹配</p><p>模板引用丢失：动态模板引用在预构建过程中被错误处理</p><p>组件样式隔离：预构建可能导致组件样式作用域失效</p><p>模板未找到的具体原因：</p><p>预构建过程中，<code>Taro</code>可能未能正确解析<code>base.wxml</code>中动态引用的模板</p><p>表达式<code>{{xs.a(5, i.nn, l)}}</code>生成的模板名称在预构建时被错误处理，导致运行时无法匹配</p><p>组件显示异常的原因：</p><p><code>AtModal</code>按钮不显示通常是因为：</p><p>组件样式被预构建过程隔离，未正确注入到页面</p><p>动态组件渲染逻辑在预构建环境中执行异常</p><p>三、解决方案原理说明</p><p>将配置修改为以下形式解决了问题：</p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js"><pre><code><span class="line"><span class="token literal-property property">compiler</span><span class="token operator">:</span> <span class="token punctuation">{</span></span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这个方案的核心是禁用<code>Webpack5</code>的预构建功能，其解决问题的逻辑如下：</p><p>关闭预构建的直接效果：</p><p>模块加载顺序恢复正常：所有模块按运行时需求动态加载，避免时序错误</p><p>模板解析回归动态模式：<code>Taro</code>在运行时按需解析模板引用，确保<code>tmpl_0_13</code>被正确查找</p><p>组件样式作用域修复：关闭预构建后，组件样式能正确注入到页面作用域</p><p>配置参数的具体作用：</p><p><code>type: &#39;webpack5&#39;</code>：保持使用<code>Webpack5</code>编译器，确保新特性支持</p><p><code>prebundle.enable</code>: <code>false</code>：禁用预构建功能，这是解决问题的关键</p><p><code>prebundle.include</code>：虽然启用了 <code>include</code>，但由于<code>enable</code>设为<code>false</code>，该配置实际不生效</p><p>更深层的技术原理：</p><p>预构建本质是为了优化性能，但在复杂项目中可能破坏动态依赖关系</p><p><code>Taro</code>的模板系统依赖运行时动态解析，预构建的静态分析无法处理所有动态引用场景</p><p>关闭预构建后，<code>Webpack</code>回归到传统的按需打包模式，确保模块和模板的动态加载能力</p>`,72)]))}const t=n(l,[["render",c]]),r=JSON.parse('{"path":"/guide/AboutTaro/","title":"Taro小程序+React——基础知识与正常运行须知","lang":"zh-CN","frontmatter":{},"headers":[{"level":2,"title":"状态管理（重点）","slug":"状态管理-重点","link":"#状态管理-重点","children":[]},{"level":2,"title":"发送请求（重点）","slug":"发送请求-重点","link":"#发送请求-重点","children":[]},{"level":2,"title":"注意事项：域名备案","slug":"注意事项-域名备案","link":"#注意事项-域名备案","children":[]},{"level":2,"title":"Webpack5 预构建机制导致的问题","slug":"webpack5-预构建机制导致的问题","link":"#webpack5-预构建机制导致的问题","children":[]}],"git":{"updatedTime":1754140421000,"contributors":[{"name":"李锐标","username":"","email":"1415808154@qq.com","commits":2},{"name":"X4M7","username":"X4M7","email":"1415808154@qq.com","commits":4,"url":"https://github.com/X4M7"}],"changelog":[{"hash":"2dc47d14f98b83220178e1ff0e47c550a7442cec","time":1754140421000,"email":"1415808154@qq.com","author":"X4M7","message":"优化taro介绍基础知识与正常运行部分的表达"},{"hash":"8e61c918405d118e4df6ba6aa7830a9067521d4d","time":1754139660000,"email":"1415808154@qq.com","author":"X4M7","message":"react+taro小程序项目文档基本完成"},{"hash":"e43bab961abd39d293883f88cdbb1b492eb3e307","time":1750524888000,"email":"1415808154@qq.com","author":"X4M7","message":"略记无时间详解收获"},{"hash":"42c34a34f2b7a907e8a01bdd5e0e6eef4ef98611","time":1750468946000,"email":"1415808154@qq.com","author":"李锐标","message":"记录没时间详解的收获"},{"hash":"6f04d3ffa0f5ae82e07d6618a3dd6590808ce70f","time":1750086280000,"email":"1415808154@qq.com","author":"X4M7","message":"补充taro预构建导致问题解决方案"},{"hash":"602e0b1be38e4c5febc7f723fe34af31371125d9","time":1749549540000,"email":"1415808154@qq.com","author":"李锐标","message":"记录taro的学习"}]},"filePathRelative":"guide/AboutTaro/README.md"}');export{t as comp,r as data};
